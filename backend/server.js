...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/register" && req.method === "POST") {
    ...
    catch { res.writeHead(400); res.end(JSON.stringify({ error: "Invalid request body." })); return; }
    ...
    db.query("SELECT user_id FROM user_account WHERE email = ?", [email], (err, rows) => {
      ...
      db.query(pSql, pVals, (err, pResult) => {
        if (err) {
          if (err.code === "ER_DUP_ENTRY") { res.writeHead(409); res.end(JSON.stringify({ error: "ID number already registered." })); }
          else { console.error("Passenger insert error:", err.code, err.message); res.writeHead(500); res.end(JSON.stringify({ error: err.message, code: err.code })); }
          return;
        }
        ...
        db.query("INSERT INTO user_account (passenger_id, email, password, role) VALUES (?,?,?,?)",
          [newPassengerId, email.trim(), password, role], (err, result) => {
            ...
            res.writeHead(201);
            res.end(JSON.stringify({ message: "Account created successfully.", user_id: result.insertId, passenger_id: newPassengerId, role }));
          });
      });
    });
    return;
  ...
  if (req.url === "/login" && req.method === "POST") {
    ...
    const email = body.email?.trim(), password = body.password?.trim(), role = body.role?.trim();
    if (!email || !password || !role) { res.writeHead(400); res.end(JSON.stringify({ error: "Email, password, and role are required." })); return; }
