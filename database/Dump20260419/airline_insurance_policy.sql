...
CREATE TABLE `insurance_policy` (
  ...
  `coverage_amount` decimal(7,2) NOT NULL,
  `insurance_fee` decimal(5,2) NOT NULL,
  `coverage_type` enum('Trip Cancellation','Medical','Baggage','Full Coverage') NOT NULL,
  PRIMARY KEY (`insurance_id`),
  KEY `booking_id` (`booking_id`),
  CONSTRAINT `insurance_policy_ibfk_1` FOREIGN KEY (`booking_id`) REFERENCES `bookings` (`booking_id`),
  CONSTRAINT `insurance_policy_chk_1` CHECK ((`coverage_amount` >= 0)),
  CONSTRAINT `insurance_policy_chk_2` CHECK ((`insurance_fee` >= 0))
...
-- Dump completed on 2026-04-19 15:14:34
